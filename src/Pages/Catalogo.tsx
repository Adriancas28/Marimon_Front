...
function Catalogo() {
  ...
  // Cargar categorías reales desde la API (una sola vez)
  useEffect(() => {
  ...
  const fetchProductos = async () => {
    ...
    try {
      ...
      // La API devuelve: { totalRegistros, totalPaginas, paginaActual, data: [...] }
      const prdArray: any[] = Array.isArray(data) ? data : (data?.data || []);

      // ✅ FIX: leer "totalPaginas" que es el campo real de la API
      if (data?.totalPaginas) setTotalPaginas(data.totalPaginas);

      const processed: Producto[] = prdArray.map((p: any) => ({
        ...p,
  ...
  // ✅ FIX: filtrar por categoriaId (número) y aplicar ordenamiento de precio
  const filteredProducts = productos
    .filter((p) => {
      const matchesSearch = p.nombre.toLowerCase().includes(searchTerm.toLowerCase());
      // ✅ FIX: comparar categoriaId numérico contra los ids seleccionados
    ...
    .sort((a, b) => {
      // ✅ FIX: aplicar ordenamiento por precio
      if (sortBy === "asc") return a.precio - b.precio;
      if (sortBy === "desc") return b.precio - a.precio;
      if (sortBy === "new") return b.id - a.id; // más reciente = id mayor
      return 0;
    });
  ...
  const getNombreCategoria = (catId?: number) => {
    ...
    return found ? found.nombre : "Repuesto Original";
  };
          ...
          <main className="flex-1">
            ...
            {!loading && !error && (
              <div className="flex items-center justify-between mb-6">
                <p className="text-sm font-semibold text-slate-500">
                  <span className="text-slate-800 font-black">{filteredProducts.length}</span> producto{filteredProducts.length !== 1 ? "s" : ""} encontrado{filteredProducts.length !== 1 ? "s" : ""}
                  {selectedCategories.length > 0 && (
                    <span className="ml-2 text-red-500">
                      · {selectedCategories.length} categoría{selectedCategories.length > 1 ? "s" : ""} activa{selectedCategories.length > 1 ? "s" : ""}
                    </span>
                  )}
                </p>
            ...
            {loading ? (
              ...
              <div className="bg-white rounded-3xl p-12 text-center border border-slate-100 shadow-sm">
                <img
                  src="https://cdn-icons-png.flaticon.com/512/5622/5622660.png"
                  alt="No products"
                        ...
                        <div className="mt-4 flex items-end justify-between">
                          ...
                          <div
                            className={`px-2 py-1 rounded-lg text-[10px] font-black uppercase ${
                              producto.stock > 0 ? "bg-green-50 text-green-600" : "bg-red-50 text-red-600"
                            }`}
                          >
